...
func TestCLIWorkflowWithDirectoryInputs(t *testing.T) {
	...
	gff3Path := filepath.Join(dir, "sample1.gff3")
	_, stderr, err = executeCommand("gff3", filepath.Join(annotationsDir, "sample1.bakta.json"), filepath.Join(genomesDir, "sample1.fa"), "-o", gff3Path)
	if err != nil {
		t.Fatalf("gff3 command error = %v, stderr = %s", err, stderr)
	}
	gff3Out, err := os.ReadFile(gff3Path)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(gff3Out, []byte("# Generated by bakpack ")) || !bytes.Contains(gff3Out, []byte("\tPyrodigal\tCDS\t")) || !bytes.Contains(gff3Out, []byte("##FASTA\n>contig1\nATGAAATAA\n")) {
		t.Fatalf("gff3 output not written correctly: %s", gff3Out)
	}

	gff3AnnotationOnlyPath := filepath.Join(dir, "sample1.annotation-only.gff3")
	_, stderr, err = executeCommand("gff3", filepath.Join(annotationsDir, "sample1.bakta.json"), filepath.Join(genomesDir, "sample1.fa"), "-o", gff3AnnotationOnlyPath, "--annotation-only")
	if err != nil {
		t.Fatalf("gff3 --annotation-only command error = %v, stderr = %s", err, stderr)
	}
	gff3AnnotationOnlyOut, err := os.ReadFile(gff3AnnotationOnlyPath)
	if err != nil {
		t.Fatal(err)
	}
	if !bytes.Contains(gff3AnnotationOnlyOut, []byte("# Generated by bakpack ")) || bytes.Contains(gff3AnnotationOnlyOut, []byte("##FASTA")) || !bytes.Contains(gff3AnnotationOnlyOut, []byte("\tPyrodigal\tCDS\t")) {
		t.Fatalf("gff3 annotation-only output not written correctly: %s", gff3AnnotationOnlyOut)
	}
	...
	gff3Extracted, err := os.ReadFile(filepath.Join(outDir, "sample1.gff3"))
	if err != nil {
		t.Fatal(err)
	}
