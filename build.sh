...
Default behavior:
  Builds bakpack for the current GOOS/GOARCH into ./build/.
...
build_one() {
	...
	(
		...
		CGO_ENABLED=0 GOOS="$goos" GOARCH="$goarch" \
			go build -trimpath \
				-ldflags "-X github.com/martinghunt/bakpack/internal/buildinfo.Version=${build_version}" \
				-o "$outfile" ./cmd/bakpack
	)
